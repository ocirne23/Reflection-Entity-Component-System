...
/**
 * An InputStream that reads data from a byte array and optionally fills the
 * byte array from another InputStream as needed. Utility methods are provided
 * for efficiently reading primitive types and strings.
 * <p>
 * The byte[] buffer may be modified and then returned to its original state
 * during some read operations, so the same byte[] should not be used
...
public class Input extends InputStream {
	 ...
	 *
	 * @param bufferSize
	 *            The size of the buffer. An exception is thrown if more bytes
	 *            than this are read.
	 */
	...
	/**
	 * Sets a new buffer. The position and total are reset, discarding any
	 * buffered bytes.
	 */
	...
	/**
	 * Sets a new InputStream. The position and total are reset, discarding any
	 * buffered bytes.
	 *
	 * @param inputStream
	 *            May be null.
	...
	/**
	 * Fills the buffer with more bytes. Can be overridden to fill the bytes
	 * from a source other than the InputStream.
	 *
	 * @return -1 if there are no more bytes.
	 */
	protected int fill(byte[] buffer, int offset, int count) throws RuntimeException {
		if (inputStream == null)
			return -1;
		try {
	...
	/**
	 * @param required
	 *            Must be > 0. The buffer is filled until it has at least this
	 *            many bytes.
	 * @return the number of bytes remaining.
	 * @throws RuntimeException
	 *             if EOS is reached before required bytes are read (buffer
	 *             underflow).
	 */
	 ...
	 * @return the number of bytes remaining, but not more than optional, or -1
	 *         if the EOS was reached and the buffer is empty.
	 */
	...
	/**
	 * Reads a single byte as an int from 0 to 255, or -1 if there are no more
	 * bytes are available.
	 */
	@Override
	...
	/**
	 * Reads count bytes or less and writes them to the specified byte[],
	 * starting at offset, and returns the number of bytes read or -1 if no more
	 * bytes are available.
	 */
	@Override
	public int read(byte[] bytes, int offset, int count) throws RuntimeException {
		if (bytes == null)
			throw new IllegalArgumentException("bytes cannot be null.");
	 ...
	 * offset.
	 */
	public void readBytes(byte[] bytes, int offset, int count) throws RuntimeException {
		if (bytes == null)
			throw new IllegalArgumentException("bytes cannot be null.");
		int copyCount = Math.min(limit - position, count);
		while (true) {
	...
	/**
	 * Reads a 1-5 byte int. It is guaranteed that a varible length encoding
	 * will be used.
	 */
	...
	/**
	 * Reads a 1-9 byte long. It is guaranteed that a varible length encoding
	 * will be used.
	 */
