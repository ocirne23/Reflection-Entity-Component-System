...
public class Output extends OutputStream {
	...
	protected boolean require (int required) throws RuntimeException {
		...
		if (required > maxCapacity)
			throw new RuntimeException("Buffer overflow. Max capacity: " + maxCapacity + ", required: " + required);
		flush();
		while (capacity - position < required) {
			if (capacity == maxCapacity)
				throw new RuntimeException("Buffer overflow. Available: " + (capacity - position) + ", required: " + required +":"+ position);
			// Grow buffer.
	...
	/** Writes the bytes. Note the byte[] length is not written. */
	@Override
	public void write (byte[] bytes, int offset, int length) throws RuntimeException {
		writeBytes(bytes, offset, length);
	}

	// byte

	public void writeByte (byte value) throws RuntimeException {
		if (position == capacity) require(1);
		buffer[position++] = value;
	}

	public void writeByte (int value) throws RuntimeException {
		if (position == capacity) require(1);
		buffer[position++] = (byte)value;
	}

	/** Writes the bytes. Note the byte[] length is not written. */
	public void writeBytes (byte[] bytes) throws RuntimeException {
		if (bytes == null) throw new IllegalArgumentException("bytes cannot be null.");
		writeBytes(bytes, 0, bytes.length);
	}

	/** Writes the bytes. Note the byte[] length is not written. */
	public void writeBytes (byte[] bytes, int offset, int count) throws RuntimeException {
		if (bytes == null) throw new IllegalArgumentException("bytes cannot be null.");
	 ...
	 *           inefficient (5 bytes). */
	public int writeInt (int value) throws RuntimeException {
		return writeVarInt(value, true);
	}
	 ...
	 * @param optimizePositive If true, small positive numbers will be more efficient (1 byte) and small negative numbers will be
	 *           inefficient (5 bytes). */
	public int writeVarInt (int value, boolean optimizePositive) throws RuntimeException {
	 ...
	 * @param value May be null. */
	public void writeString (CharSequence value) throws RuntimeException {
		if (value == null) {
			writeByte(0x80); // 0 means null, bit 8 means UTF8.
			return;
		}
	 ...
	 * @param value May be null. */
	@SuppressWarnings("deprecation")
	public void writeAscii (String value) throws RuntimeException {
		if (value == null) {
			writeByte(0x80); // 0 means null, bit 8 means UTF8.
			return;
		}
	...
	@SuppressWarnings("deprecation")
	private void writeAscii_slow (String value, int charCount) throws RuntimeException {
		byte[] buffer = this.buffer;
		int charIndex = 0;
	 ...
	 * @param optimizePositive If true, small positive numbers will be more efficient (1 byte) and small negative numbers will be
	 *           inefficient (5 bytes). */
	public int writeFloat (float value, float precision, boolean optimizePositive) throws RuntimeException {
		return writeInt((int)(value * precision));
	}
	 ...
	 * @param optimizePositive If true, small positive numbers will be more efficient (1 byte) and small negative numbers will be
	 *           inefficient (9 bytes). */
	public int writeLong (long value) throws RuntimeException {
		return writeVarLong(value, true);
	}

	/** Writes a 1-9 byte long. It is guaranteed that a varible length encoding will be used.
	 * @param optimizePositive If true, small positive numbers will be more efficient (1 byte) and small negative numbers will be
	 *           inefficient (9 bytes). */
	public int writeVarLong (long value, boolean optimizePositive) throws RuntimeException {
	 ...
	 * @param optimizePositive If true, small positive numbers will be more efficient (1 byte) and small negative numbers will be
	 *           inefficient (9 bytes). */
	public int writeDouble (double value, double precision, boolean optimizePositive) throws RuntimeException {
		return writeLong((long)(value * precision));
	}
	...
	public void writeInts (int[] object) throws RuntimeException {
		for (int i = 0, n = object.length; i < n; i++)
			writeInt(object[i]);
	}

	/** Bulk output of an long array. */
	public void writeLongs (long[] object) throws RuntimeException {
		for (int i = 0, n = object.length; i < n; i++)
			writeLong(object[i]);
	}

	/** Bulk output of a float array. */
	public void writeFloats (float[] object) throws RuntimeException {
		for (int i = 0, n = object.length; i < n; i++)
			writeFloat(object[i]);
	}

	/** Bulk output of a short array. */
	public void writeShorts (short[] object) throws RuntimeException {
		for (int i = 0, n = object.length; i < n; i++)
			writeShort(object[i]);
	}

	/** Bulk output of a char array. */
	public void writeChars (char[] object) throws RuntimeException {
		for (int i = 0, n = object.length; i < n; i++)
			writeChar(object[i]);
	}

	/** Bulk output of a double array. */
	public void writeDoubles (double[] object) throws RuntimeException {
		for (int i = 0, n = object.length; i < n; i++)
