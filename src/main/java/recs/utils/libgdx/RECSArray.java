...
public class RECSArray<T> implements Iterable<T> {
	...
	public RECSArray (boolean ordered, int capacity) {
	 ...
	 *           memory copy.
	 * @param capacity Any elements added beyond this will cause the backing array to be grown. */
	public RECSArray (boolean ordered, int capacity, Class<T> arrayType) {
		this.ordered = ordered;
	...
	public RECSArray (Class<T> arrayType) {
	...
	public RECSArray (RECSArray array) {
	 ...
	 * @param ordered If false, methods that remove elements may change the order of other elements in the array, which avoids a
	 *           memory copy. */
	public RECSArray (boolean ordered, T[] array) {
	...
	public void addAll (RECSArray array) {
		addAll(array, 0, array.size);
	}

	public void addAll (RECSArray array, int offset, int length) {
	...
	public boolean removeAll (RECSArray<T> array, boolean identity) {
		int size = this.size;
		int startSize = size;
		T[] items = this.items;
		if (identity) {
	...
	public boolean equals (Object object) {
		...
		if (!(object instanceof RECSArray)) return false;
		RECSArray array = (RECSArray)object;
		int n = size;
	...
	static public class ArrayIterator<T> implements Iterator<T> {
		private final RECSArray<T> array;
		int index;

		public ArrayIterator (RECSArray<T> array) {
			this.array = array;
		}
	...
	static public class ArrayIterable<T> implements Iterable<T> {
		...
		public ArrayIterable (RECSArray<T> array) {
